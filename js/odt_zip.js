  // Helper para guardar ZIP usando File Picker si disponible, o APIs nativas de Tauri si estamos en app de escritorio
  async function saveZipWithPicker(blob, defaultName){
    const name = defaultName || 'expediente.zip';

    // --- Rama Tauri (app escritorio: macOS / Windows / Linux) ---
    try{
      if (window.__TAURI__ && window.__TAURI__.dialog && window.__TAURI__.fs){
        console.log('[ODT] Usando rama Tauri para guardar ZIP');
      ...
      console.warn('saveZipWithPicker: rama Tauri falló, se continúa con flujo web estándar.', e);
      // en caso de error, continuamos con el flujo normal de navegador
    }

    // --- Rama navegador: File Picker nativo si existe ---
    ...
    // --- Fallback clásico: descarga directa en navegador ---
...
(function(){
  ...
  async function exportarODTJSON(){
    ...
    try{ console.log('[ODT] exportarODTJSON llamado'); alert('DEBUG: exportarODTJSON llamado'); }catch(_){}
...
if (sexo){
  ...
  window.CompaExportDetenidoODTJSON = async function(det, index){
    try{ console.log('[ODT] CompaExportDetenidoODTJSON llamado'); }catch(_){}
    if (!det) {
